...
export default class UserService {
    constructor() {
        ...
        axios.interceptors.response.use(null, error => {
            if (error.config && error.response && error.response.status === 401) {
                ...
                axios.post(this.tokenEndpoint, formData, refreshTokenConfig)
                    .then(response => {
                        ...
                        error.config.headers['Authorization'] = 'Bearer ' + this.cookie.get('access_token');
                        window.location.reload();
                    }).catch(error => {
    ...
    saveTokens(tokenResponse) {
        ...
        refreshTokenExpireDate.setTime(refreshTokenExpireDate.getTime() + 24 * 60 * 60 * 1000);
